...
import gc
import time
from apps.yolov8n_seg.core.yolov8n_segment import YoloSegDemo, ConfigManager
...
class App(BaseApp):
    def __init__(self, app_manager):
        super().__init__(app_manager, name="图像分割", icon=None)
        try:
            with open("/sdcard/apps/yolov8n_seg/dock_icon.png", 'rb') as f:
                dock_data = f.read()
                self.dock_icon_data = dock_data
        except:
            self.dock_icon_data = None
        try:
            with open("/sdcard/apps/yolov8n_seg/icon.png", 'rb') as f:
                icon_data = f.read()
    ...
    def _camera_loop(self):
        ...
        mask_threshold = self.cfg_mgr.get("mask_threshold", 0.5)

        try:
            self.demo = YoloSegDemo(self.pl)
            self.demo.exce_demo_init(confidence_threshold, nms_threshold, mask_threshold)
        ...
        while self.is_running:
            try:
                ...
                if not is_panel:
                    ...
                    if img is not None:
                        seg_res = self.demo.run(img)
                        self._ui_img.clear()
                        self.demo.draw_result(self._ui_img, seg_res)
                        self._draw_title_bar(self._ui_img)
                else:
                    self._ui_img.clear()
    ...
    def _draw_title_bar(self, ui_img, title="图像分割"):
    ...
    def _draw_settings_menu(self, ui_img):
        ...
        gap = 20
        cards = [
            ("detect", "检测设置", "置信度/NMS/掩码", LAYOUT['card_icon_detect']),
        ]
    ...
    def _draw_detect_panel(self, ui_img):
        ...
        # NMS阈值
        nms_y = ty + 95
        ui_img.draw_string_advanced(pad, nms_y, LAYOUT['font_md'], "NMS: %.2f" % self._tmp_nms, color=LAYOUT['text_dim'])
        ...
        ui_img.draw_line(pad, nms_y + 85, 640 - pad, nms_y + 85, color=LAYOUT['divider'], thickness=1)

        # 掩码阈值
        mask_y = nms_y + 95
        ui_img.draw_string_advanced(pad, mask_y, LAYOUT['font_md'], "掩码: %.2f" % self._tmp_mask, color=LAYOUT['text_dim'])
        ui_img.draw_rectangle(pad + 10, mask_y + 30, cw, ch, color=LAYOUT['danger_dark'], thickness=1, fill=True)
        ui_img.draw_rectangle(pad + 10, mask_y + 30, cw, ch, color=LAYOUT['danger'], thickness=1)
        ui_img.draw_string_advanced(pad + 10 + (cw - LAYOUT['font_md']) // 2, mask_y + 30 + (ch - LAYOUT['font_md']) // 2, LAYOUT['font_md'], "-", color=LAYOUT['text_white'])
        bar_x3 = pad + 10 + cw + 10
        ui_img.draw_rectangle(bar_x3, mask_y + 38, bar_w, 12, color=LAYOUT['bar_bg'], thickness=1, fill=True)
        fill_w3 = max(1, int(bar_w * self._tmp_mask))
        ui_img.draw_rectangle(bar_x3, mask_y + 38, fill_w3, 12, color=LAYOUT['bar_fg'], thickness=1, fill=True)
        ui_img.draw_rectangle(rx - 10, mask_y + 30, cw, ch, color=LAYOUT['confirm_dark'], thickness=1, fill=True)
        ui_img.draw_rectangle(rx - 10, mask_y + 30, cw, ch, color=LAYOUT['confirm'], thickness=1)
        ui_img.draw_string_advanced(rx - 10 + (cw - LAYOUT['font_md']) // 2, mask_y + 30 + (ch - LAYOUT['font_md']) // 2, LAYOUT['font_md'], "+", color=LAYOUT['text_white'])
    ...
    def _open_detect_panel(self):
        ...
        self._tmp_mask = float(self.cfg_mgr.get("mask_threshold", 0.5))
        self.panel = "detect"
    ...
    def _handle_detect_touch(self, x, y):
        ...
        # 掩码 -
        mask_y = nms_y + 95
        if self._in_rect(x, y, pad + 10 - te, mask_y + 30 - te, cw + 2 * te, ch + 2 * te):
            self._tmp_mask = max(0.05, min(1.0, round(self._tmp_mask - 0.05, 2)))
            return
        # 掩码 +
        if self._in_rect(x, y, rx - 10 - te, mask_y + 30 - te, cw + 2 * te, ch + 2 * te):
            self._tmp_mask = max(0.05, min(1.0, round(self._tmp_mask + 0.05, 2)))
            return
        ...
        if self._in_rect(x, y, 160 - te, btn_y - te, 140 + 2 * te, 48 + 2 * te):
            ...
            self.cfg_mgr.set("mask_threshold", self._tmp_mask)
            try:
                self.demo = YoloSegDemo(self.pl)
                self.demo.exce_demo_init(self._tmp_conf, self._tmp_nms, self._tmp_mask)
            except:
                pass
            self.panel = None
            return
